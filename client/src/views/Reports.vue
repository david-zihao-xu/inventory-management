              ...
              <tr v-for="(month, index) in monthlyData" :key="month.month">
                ...
                <td>
                  <span v-if="index > 0" :class="getChangeClass(month.revenue, monthlyData[index - 1].revenue)">
                    {{ getChangeDisplay(month.revenue, monthlyData[index - 1].revenue) }}
                  </span>
                  <span v-else>-</span>
                </td>
                <td>
...
export default {
  ...
  setup() {
    ...
    const getBarHeight = (revenue) => {
      const maxRevenue = monthlyData.value.reduce((max, m) => Math.max(max, m.revenue || 0), 0)
    ...
    // Renders the change value with +/- sign and currency formatting (no manual $)
